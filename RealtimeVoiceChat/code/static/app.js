...
function initAudioContext() {
  if (!audioContext) {
    // Force 48kHz so server-side resample ratio (48k -> 16k) is correct
...
async function startRawPcmCapture() {
  try {
    const stream = await navigator.mediaDevices.getUserMedia({
      audio: {
        // Match AudioContext at 48kHz to avoid browser resampling artifacts
...
function handleJSONMessage({ type, content }) {
  ...
  if (type === 'tts_chunk') {
    ...
    console.log('tts_chunk samples', int16Data.length);
    if (ttsWorkletNode) {
      ttsWorkletNode.port.postMessage(int16Data);
    } else {
      console.warn('No ttsWorkletNode when tts_chunk arrived');
    }
    return;
  }
